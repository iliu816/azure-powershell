...
Restart specified nodes on the node type.
Nodes will be restarted in batches according to their assigned UD.
If all selected nodes are in the same UD, they will be restarted at the same time.
...
Restart all nodes on the node type.
Nodes will be restarted in batches according to their assigned UD.
